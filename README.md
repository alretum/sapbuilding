...
- **Leaderboard**: the live snapshot includes every employee ranked by points company-wide (shown on the dashboard). A cross-company board lives at `/leaderboard`.
- **Readiness map** (`/map`): a stylized SVG choropleth of Germany (react-simple-maps) coloured by regional readiness; zoom into a region to reveal glowing city bubbles. SAP sets a company's region/city at setup; cities only show a score with ≥ 3 companies (k-anonymity). Seed demo data with `npm run db:seed:map`.
